...
vl_sims_vrc01 %>%
  ...
  ggplot(aes(x = days_fp, y = pmax(1, log10V))) +
  geom_line(aes(group = pub_id), alpha = 0.25) +
  facet_grid(cols = vars(ic80_cat), rows = vars(sim_type))
...
sims_out = vl_sims_vrc01 %>%
  mutate(days_fp = days - fp_day) %>%
  filter(days_fp > -30 & days >= model_infection_day) %>% 
  select(pub_id, sim_type, days_fp, log10V, ic80_cat, contains("gmt"), first_positive_row, fp_day, model_infection_day, days_enrollment = days)
