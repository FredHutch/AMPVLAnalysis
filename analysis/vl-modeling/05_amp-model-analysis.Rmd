...
vl_data_summary = vl_data %>%
  ...
  summarize(
    ...
    n_setpt = sum(days_fp > days_fp[which.max(log10vl)] & cens <= 0),
    setpt = if_else(n_setpt == 0, NA_real_, mean(log10vl[days_fp > days_fp[which.max(log10vl)] & cens <= 0]))
  )
...
final_vl_summary = full_join(vl_summary, vl_data_summary, by = "pub_id")
