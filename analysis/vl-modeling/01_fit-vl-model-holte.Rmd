...
Here, we fit the RV217 Holte models to the AMP participants. Except for the infection time parameters, all of the 
fixed population parameters are fixed from the RSIF paper. For the omegas, there are three approaches: 1) fix all of the omegas, 2) refit the omegas, and 3) refit omegas with placebos only.
...
The major models:
**All models refit infection time parameter and omega.**
...
Three sub-models:
A) Fit using all (trt+placebo) participants (denoted _all)
B) Fit using placebo only (denoted _placebo)
C) Placebo model applied to vrc01 groups (denoted _vrc01)
...
save_project(vl_dir_tag("holte_vl_corr_placebo_vrc01"))
...
set_holte_parms(holte_parms = rv217_fits, est_omega = "MLE")
...
save_project(vl_dir_tag("holte_vl_omega_all"))
...
save_project(vl_dir_tag("holte_vl_omega_placebo"))

```

```{r rsif-refit-omega--trt, eval = F}

placebo_refitomega = get_pop_ests(vl_dir_tag("holte_vl_omega_placebo")) %>%
  select(parameter, value) %>% spread(parameter, value)
...
set_holte_parms(holte_parms = placebo_refitomega)
setPopulationParameterInformation(
...
dI correlations are minor and poorly specified (high RSEs) so I remove all 3. After fixing the constant error 
(because the se is always NaN) and refitting the omega dS was poorly specified (NaN, standard error). Then I tried
linearization and that seemed to work well, none of the rses shrunk and omega dS se was estimated.
