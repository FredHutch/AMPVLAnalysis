...
amp_theta_pd = tibble(IC80 = NA, rho = 550, h = 1.25)
...
placebo_pop_ests = stack_model_ests(adj_placebo_model_loc)
placebo_pop_wide = placebo_pop_ests %>%
  dplyr::select(parameter, value) %>% spread(parameter, value)

theta_vl_placebo = prep_vl_model_parms(mlx_project = adj_placebo_model_loc) %>%
  dplyr::select(-initT)

burst_effect = function(ic80, pop_parms = placebo_pop_wide) {
  lp = placebo_pop_wide$lp_pop + placebo_pop_wide$beta_lp_log_ic80_ctr * (log10(ic80) - 0.638)
  10^(lp)
}
...
vrc01_pop_ests = stack_model_ests(
  bind_rows(pkpd_models_pkpd_only, pkpd_models_final)$model_location,
  bind_rows(pkpd_models_pkpd_only, pkpd_models_final)$model_label
  )
vrc01_pop_ests_wide = vrc01_pop_ests %>%
  dplyr::select(model, parameter, value) %>% spread(parameter, value)
...
theta_vl_vrc01 = prep_vl_model_parms(mlx_project = pkpd_models_final$model_location) 
...
pk_sim_data = read_csv(here("output/pk-modeling/sim-pk-withVLneut.csv"), col_types = cols())
conc_range = range(pk_sim_data$centr_conc, na.rm = T)
