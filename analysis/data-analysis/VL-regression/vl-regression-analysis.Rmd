...
For the GEE analysis, I had envisioned following the approach laid out in your and Holly’s nature med paper on multiple founders and viral loads. There were three VL outcomes based on that paper that I thought would be relevant for this study:
- log10 VL at first positive
- mean log10 VL based on longitudinal data up to 3 months. This would use GEE with IPW
- setpoint log10 VL (mean log10 VL 8-12 weeks post-infection). We could use multiple imputation or impute from the mathematical models, would be curious thoughts on this?
Maybe we could use Friday’s meeting to discuss model specification regarding other relevant variables to include (ex, total isolates, IC80 category)


PG: To minimize potential post-randomization selection bias, generally we'd try to use baseline covariate adjustment in all models assessing different viral loads across randomized arms.  Baseline variables that dually predict viral load and HIV-1 acquisition cause selection bias.  There are not smoking gun baseline covariates to be concerned about.  Generally for all baseline covariate adjusted analyses we have adjusted for study/region, which we have defined as having levels
(1) South Africa/703, (2) not South Africa/703, (3) US or Switzerland/704, (4) South America/704.  Its advantageous to use this same type of adjustment, just for consistency across papers.  It probably won't make much difference. 
...
mod_fp_interaction = lm(log10vl ~ study_region_cat + rx_lab * ic80gte1, data = first_positive)
mod_fp_main_effects = lm(log10vl ~ study_region_cat + rx_lab * ic80gte1, data = first_positive)
...
emmeans(mod_fp_interaction, ~ ic80gte1+rx_lab) %>%
  as_tibble() %>%
  ggplot(aes(x = rx_lab, y = emmean, ymin = lower.CL, ymax = upper.CL)) +
  geom_pointrange() +
...
mod_fp_pool_interaction = lm(log10vl ~ study_region_cat + rx_pool * ic80gte1, data = first_positive)
