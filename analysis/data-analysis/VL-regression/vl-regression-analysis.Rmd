  ...
  tibble(grps = combn(1:n_grps, 2, paste, collapse = '_')) %>%
    ...
    mutate(
      contrast_res = map2(grp1, grp2, ~as_tibble(
        ...
        rownames = "contrast")),
      pvalue =  map2_dbl(grp1, grp2, ~wald_test(drtmle_obj, 
...
drtmle_fit_fp = drtmle(Y = outcome, A = trt_groups, W = cov,
             stratify = FALSE,
             SL_Q = c("SL.glm", "SL.mean", "SL.bayesglm", "SL.glmnet"),
             SL_g = c("SL.glm", "SL.mean", "SL.bayesglm", "SL.step"),
             guard = NULL,
             #glm_Qr = "gn",
             #glm_gr = "Qn",
             ...
             cvFolds = 10,
             SL_method = "method.NNLS", family = gaussian(), returnModels = F)

        
# compare point estimates for all different estimators
drtmle_fit_fp_ci = ci(drtmle_fit_fp, est = c( "tmle", "aiptw", "gcomp", "drtmle"))
...
emmeans(mod_fp_interaction, pairwise ~ trt_ic80, adjust = "none") 
get_drtmle_contrasts(drtmle_fit_fp) %>% left_join(outcome_map, by = c("grp1" = "trt_ic80_code")) %>% left_join(outcome_map, by = c("grp2" = "trt_ic80_code"), suffix = c("_grp1", "_grp2"))
```
...
emmeans(mod_fp_pool_interaction, pairwise ~ trt_ic80_pool, adjust = "none") 
