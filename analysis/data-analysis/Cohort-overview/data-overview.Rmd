...
vl_data_final = read_csv(clean_data_here("adata-vl.csv"), col_types = cols())
...
study_region_map = distinct(vl_data_final, study_region_code, study_region_cat)

demo_vars = distinct(vl_data_all, pub_id, study_region_code, any_art, protocol) %>%
  left_join(rx_map, by = "pub_id") %>%
  mutate(study_region_cat = factor(study_region_code, 
                                   levels = study_region_map$study_region_code,
                                   labels = study_region_map$study_region_cat),
         any_art = factor(any_art, levels = c(F, T), labels = c("No", "Yes")))

neut_vars = distinct(vl_data_final, pub_id, nisolates, gmt80ls) %>%
...
table1_prep = measurement_summary %>%
  dplyr::filter(key %in% c("total_measurements", "total_pos_offart")) %>%
  dplyr::select(pub_id, key, total) %>%
  pivot_wider(names_from = "key", values_from = "total") %>%
  dplyr::mutate(across(c(total_pos_offart, total_measurements), 
...
label(table1_prep$total_pos_offart) = "Total positive VL<br>(pre-ART)"


var_formula = "~ study_region_cat + any_art + ic80gte1 + nisolates + total_measurements + total_pos_offart"
...
tmp = subset(vl_data_all, pub_id %in% dx_visits_afterd22$pub_id & days_dx > 0)
...
vl_data_dx_cut = vl_data_all %>%
  ...
  left_join(neut_vars, by = "pub_id") %>%
  left_join(dplyr::select(demo_vars, pub_id, rx_lab), by = "pub_id") %>%
  mutate(
