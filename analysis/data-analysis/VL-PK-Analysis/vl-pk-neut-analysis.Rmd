...
vl_sims_infection_stop = map_df(1:nrow(filter(vl_mod_vrc01, vl_model_flag)), function(i){
  ...
  map2_df(list(dosing, dosing_stop, dosing_none),
          c("VRC01", "stop_dose_infection", "no_VRC01"),
...
```{r}
subset(vl_sims_infection_stop, !first_positive_row) %>% select(sim_type, log10V)
```

```{r}

vl_sims_infection_stop %>%
  dplyr::filter(first_positive_row) %>%
...
vl_sims_infection_stop %>%
  filter(first_positive_row & sim_type != "stop_dose_infection") %>%
  mutate(sens = gmt80ls < 1) %>%
  ggplot(aes(x = sim_type, y = log10V)) +
  geom_boxplot() +
  geom_point() +
  geom_path(aes(group = pub_id), alpha = 0.5) +
  scale_x_discrete(limits = c("VRC01", "no_VRC01")) +
