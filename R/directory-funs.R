...
.amp_model_here <- function(file = "", list_files = F) {
  if(list_files) list.files(here("inst", "extdata", "structural-model"))
  here("inst", "extdata", "structural-model", file)
}
...
.mlx_model_here <- function(file = "", mlxtran = F) {
  ...
  here("inst", "extdata", "mlx-models", file)
}
